...
(function(window, document, $, app, undefined) {
  ...
  app.init = function() {
    ...
    $(".cmb2-wrap > .cmb2-metabox")
      ...
      .on(
        "click",
        ".attached-posts-wrap .retrieved .add-remove",
        app._moveRowToAttached
      )
      // Remove posts when the minus icon is clicked
      .on(
        "click",
        ".attached-posts-wrap .attached .add-remove",
        app._removeRowFromAttached
      )
      //全選択
      .on("click", ".attached-posts-select-button", app._allListsSelect)

      //全選択解除
      .on("click", ".attached-posts-remove-button", app._allListsRemove)
  ...
  //全選択
  app._allListsSelect = function() {
    var ul_lists = $(this).next("ul");
    var li = $(ul_lists).children();
    li.each(function(index, element) {
      app.moveRowToAttached($(element));
    });
  };

  //全選択解除
  app._allListsRemove = function() {
    var ul_lists = $(this).next("ul");
    var li = $(ul_lists).children();
    li.each(function(index, element) {
      app.removeRowFromAttached($(element));
    });
  };
  ...
  app.rowTmpl = function(row) {
    row.type = app.doType
      ? ' &mdash; <span class="object-label">' + row.type + "</span>"
      : "";
    return (
      '<li data-id="' +
      row.id +
    ...
    moveInserted: function(ids) {
      ...
      setTimeout(function() {
        for (var i = 0; i <= ids.length; i++) {
          app.moveRowToAttached(
            app.$retrievedPosts().filter('[data-id="' + ids[i] + '"]')
          );
        }
      }, 500);
    }
